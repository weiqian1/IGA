%%%%% This is an example to run IGA for sparse logistic regression 
%%%%% (binary response 1 or -1)
...
wt = ones(K,1); % incoporate expert opinion on variable importance
fntemp = ceil(length(FG_true)*3/5);
wt(FG_true(1:fntemp)) = 3;  % wt > 1 corresponds to expert set
nu_cand = (6-(1:5))*.2;
cvres = IGAcv(X,y,ftype,altype,Kfold,standard,G,eps,flag,nu_cand,wt,0.005);
% estimated coefficients
path = cvres.res.solpath;
beta0 = cvres.beta0;
beta = cvres.beta;    
...
flag = 1;
eps = 20;
cvres = IGAcv(X,y,ftype,altype,Kfold,standard,G,eps,flag,1,ones(K,1),0.01);
% estimated coefficients
path = cvres.res.solpath;
beta0 = cvres.beta0;
beta = cvres.beta;
